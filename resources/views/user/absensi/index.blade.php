                        ...
                        <div class="mb-3">
                            <table class="table table-hover text-center">
                                ...
                                <td>
                                    ...
                                    $absenPulangDisplayed = false; // variabel untuk melacak apakah tombol sudah
                                    @endphp

                                    @foreach ($absensi as $data)
                                    @if ($data->tanggal_absen == $today && is_null($data->jam_keluar) &&
                                    !$absenPulangDisplayed)
                                    <form action="{{ route('welcome.update', $data->id) }}" method="POST"
                                        ...
                                        <button type="submit" class="btn btn-outline-warning" style="width: 200px"
                                            id="btnAbsenPulang-{{ $data->id }}">Absen
                                            Pulang</button>
                                    </form>
                                    @php
